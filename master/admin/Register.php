...
class Register
{
	...
	public function uniqueName()
	{
		if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) ) {
		    if('xmlhttprequest' == strtolower($_SERVER['HTTP_X_REQUESTED_WITH'])){
		        ...
		        $sql = "SELECT count(*) FROM human WHERE username = '".$this->username."'";
	...
	public function uniqueEmail()
	{
		if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) ) {
		    if('xmlhttprequest' == strtolower($_SERVER['HTTP_X_REQUESTED_WITH'])){
		        ...
		        $sql = "SELECT count(*) FROM human WHERE email = '".$this->email."'";
		        $count = mysqli_fetch_row($this->db->query($sql))[0];
		        if ($count) {
		        	echo "1";
		        }else{
	...
	public function doRegister()
	{
		...
		$this->checkName();
		$this->checkEmail();
		$sql = "INSERT INTO human (username, email, password) VALUES ('".$this->username."','".$this->email."','".$this->pwd."')";
		$result = $this->db->query($sql);
		if ($result) {
            $sql1 = "INSERT INTO punch (userid) VALUES ((select userid from human where username='".$this->username."' and email='".$this->email."'))";
            //echo "<script>alert('INSERT INTO punch (userid) VALUES ((select userid from human where username=".$this->username." and email=".$this->email."))')</script>";
            $result1 = $this->db->query($sql1);
            //
            $this->db->close();
            if ($result1){
                echo "<script>alert('Successful registration, please log in!');location.href = '/master/welcome.php';</script>";
            }
			exit();
		}else{
			echo $this->db->error;
